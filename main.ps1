...
if (-not $script:ModuleRoot) { # Fallback if $PSScriptRoot is not available (e.g. ISE or direct paste)
...
# Load stubs first to provide missing functions
. "$script:ModuleRoot\stubs.ps1"         # Function stubs for missing modules
# Helper must be first for Get-WeekStart used in core-data.ps1's $script:Data init
. "$script:ModuleRoot\helper.ps1"        # Actual filename
. "$script:ModuleRoot\core-data.ps1"     # Actual filename
...
Load-UnifiedData      # First - populates $script:Data from file, uses Get-DefaultSettings if needed
Initialize-ThemeSystem # Second - can now access $script:Data.Settings.Theme correctly
...
$script:QuickActionMap = @{
    '9' = { Add-ManualTimeEntry; return $true }; 'm' = $script:QuickActionMap['9']; 'time' = $script:QuickActionMap['9']
    's' = { Start-Timer; return $true }; 'timer' = $script:QuickActionMap['s']
    'stop' = { Stop-Timer; return $true }
    'a' = { Add-TodoTask; return $true }; 'task' = $script:QuickActionMap['a']
    ...
    't' = { Show-TodayView; return $true } # Changed from TaskManagementMenu to TodayView for dashboard shortcut
    'tasks' = { Show-TaskManagementMenu; return $true } # New +tasks for full task menu
    'w' = { Show-WeekReport; Write-Host "`nPress Enter..."; Read-Host; return $true }; 'week' = $script:QuickActionMap['w']
    'p' = { Show-ProjectDetail; Write-Host "`nPress Enter..."; Read-Host; return $true }
    'projects' = { Show-ProjectsMenu; return $true } 
    'c' = { Manage-CommandSnippets; return $true }; 'cmd' = $script:QuickActionMap['c']; 'snippets' = $script:QuickActionMap['c']
    'r' = { Show-ReportsMenu; return $true }; 'reports' = $script:QuickActionMap['r']
    'export' = { Export-FormattedTimesheet; return $true }; 'timesheet' = $script:QuickActionMap['export']
    'h' = { Show-Help; return $true }; 'help' = $script:QuickActionMap['h']
    '?' = { Show-QuickActionHelp; return $true }
    'cal' = { Show-Calendar; return $true} # Added Calendar quick action
}
...
function global:Start-UnifiedProductivitySuite {
    ...
    if (-not $script:Data.Settings.ContainsKey('QuickActionTipShown') -or -not $script:Data.Settings.QuickActionTipShown) {
        Write-Host "`nTIP: Use '+' followed by a keyword (e.e.g., +time, +task, +help) from any prompt for quick actions!" -ForegroundColor Yellow
