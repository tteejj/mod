...
function global:Show-SnippetCategories {
    ...
    foreach ($catGroup in $categories) { 
        $categoryName = if ([string]::IsNullOrEmpty($catGroup.Name)) {"[Uncategorized]"} else {$catGroup.Name} # Corrected
        Write-Host "  $categoryName: $($catGroup.Count) snippet(s)"
        
        $topInCategory = $catGroup.Group | Sort-Object UseCount -Descending, Description | Select-Object -First 3
...
function global:Show-TasksView {
    ...
    if ($Filter) {
        $tasksToDisplay = $tasksToDisplay | Where-Object {
            ...
            ($_.ProjectKey -and (Get-ProjectOrTemplate $_.ProjectKey).Name -like "*$Filter*") -or
            $_.Id -like "*$Filter*" 
        }
    }
    ...
    $sortedTasks = switch ($SortBy.ToLower()) { 
        "smart" {
            ...
            }}, @{Expression={if([string]::IsNullOrEmpty($_.DueDate)) {[DateTime]::MaxValue} else {[DateTime]::Parse($_.DueDate)}}}, 
               @{Expression={if([string]::IsNullOrEmpty($_.CreatedDate)) {[DateTime]::MinValue} else {[DateTime]::Parse($_.CreatedDate)}}}
        }
        "priority" {
            $tasksToDisplay | Sort-Object @{e={
                switch($_.Priority) {
            ...
            }}, @{Expression={if([string]::IsNullOrEmpty($_.DueDate)) {[DateTime]::MaxValue} else {[DateTime]::Parse($_.DueDate)}}}, CreatedDate
        }
        "duedate" { $tasksToDisplay | Sort-Object @{Expression={if([string]::IsNullOrEmpty($_.DueDate)) {[DateTime]::MaxValue} else {[DateTime]::Parse($_.DueDate)}}}, Priority }
        "created" { $tasksToDisplay | Sort-Object @{Expression={if([string]::IsNullOrEmpty($_.CreatedDate)) {[DateTime]::MinValue} else {[DateTime]::Parse($_.CreatedDate)}}}; Descending = $true } 
...
.UseCount}; Descending = $true}, @{Expression = {if(
# Core Data Management Module
# Projects, tasks, todos, and command snippets
...
.LastUsed}catch{[datetime]::MinValue}}else{[datetime]::MinValue}}; Descending = $true}, Description
