...
function global:Format-TableUnicode {
    param(
        ...
        [switch]$NoHeader,
        [switch]$Wrap, 
        [int]$MaxWidth = 0, # If 0, auto-width based on content and console width
        [hashtable]$RowHighlightRules = @{}, # e.g., @{ Condition = { $_.Status -eq "Overdue" }; FG = "Red" }
        [scriptblock]$RowColorCondition # Legacy, prefer RowHighlightRules
    )
    ...
    begin {
        ...
        if ($MaxWidth -eq 0) {
            try { $MaxWidth = $Host.UI.RawUI.WindowSize.Width - 2 } # Auto-set to console width if not specified
            catch { $MaxWidth = 80 } # Fallback if console width detection fails
        }
    }
    ...
    process {
        ...
        if (-not $Columns) {
            # Auto-generate columns if not provided, from first item's properties
            $props = $allData[0].PSObject.Properties | Where-Object { $_.MemberType -eq 'NoteProperty' }
            $Columns = $props | ForEach-Object {
                @{ Name = $_.Name; Title = $_.Name; Width = 0 } # Width 0 for auto-size
            }
        }
        ...
        foreach ($col in $Columns) {
            if ($null -eq $col.Width -or $col.Width -eq 0) {
                ...
                # Add padding, cap at a reasonable max, ensure minimum
                $col.Width = [Math]::Min($maxLen + 2, ($MaxWidth / 2)) # Cap individual columns
                if ($col.Width -lt ($col.Title.Length + 2)) {$col.Width = $col.Title.Length + 2} # Ensure title fits
                if ($col.Width -lt 5) { $col.Width = 5 } # Absolute minimum
            }
        }
        ...
        if ($totalCurrentWidth -gt $MaxWidth) {
            $excessWidth = $totalCurrentWidth - $MaxWidth
            # Distribute reduction proportionally, or more sophisticated logic
            # Simple proportional reduction:
            $reductionFactor = ($MaxWidth - ($Columns.Count + 1)) / ($Columns | Measure-Object -Property Width -Sum).Sum
            if($reductionFactor -lt 1 -and $reductionFactor -gt 0){ # Only shrink
                foreach ($col in $Columns) {
                    $newWidth = [Math]::Max(5, [int]($col.Width * $reductionFactor)) # Min width 5
                    $col.Width = $newWidth
                }
            }
            # Recalculate for final check (might still exceed if min widths are too large)
            $totalCurrentWidth = ($Columns | Measure-Object -Property Width -Sum).Sum + ($Columns.Count + 1)
        }
        ...
        if ($Title) {
            $titleAreaWidth = $totalCurrentWidth - 2 # Width inside borders
        ...
        foreach ($item in $allData) {
            ...
            if ($RowColorCondition -and (& $RowColorCondition $item)) { # Legacy condition
            ...
            } elseif ($rowIndex % 2 -eq 1 -and (-not $RowHighlightRules -or $RowHighlightRules.Count -eq 0)) { # AltRow if no highlight rule matched
